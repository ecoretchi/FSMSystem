...
namespace AiGame.FSM
{
    ...
    {
        EventName eventName;
        FSMEventName performEventID;
        public FSMEventCondition(EventName en, FSMEventName performEventID)
            : base(en.Name)
        {
            this.eventName = en;
            this.performEventID = performEventID;
            EventsMessanger.SubscribeToEvent(eventName, OnEvent);
        }
        ~FSMEventCondition()
        {
            Remove();
        ...
        {
           EventsMessanger.UnsubscribeFromEvent(eventName, OnEvent);
        }

        void OnEvent(EventData ed)
        {
            EventsMessanger.InvokeEvent(performEventID, new FSMEventData
            {
                transitionName = Name,
                ed = ed
            });
